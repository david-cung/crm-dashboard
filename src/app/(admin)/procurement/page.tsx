    ...
    return (
        <div className="space-y-8 max-w-7xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 className="text-3xl font-black tracking-tight text-slate-900 flex items-center gap-3">
                        ...
                        {t("procurement") || "Mua hàng"}
                    </h1>
                    <p className="text-slate-500 font-medium mt-2 flex items-center gap-2">
                        <Activity className="w-4 h-4 text-violet-500" />
                        Quản lý đơn mua hàng & nhà cung cấp
                    </p>
                </div>
                ...
                <div className="flex items-center gap-3 bg-white p-2 rounded-[24px] border border-slate-200 shadow-sm">
                    ...
                    <button onClick={() => setIsSupplierOpen(true)}
                        ...
                        <Users className="w-4 h-4" /> Thêm NCC
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <MetricCard title="Tổng đơn PO" value={stats.total_pos} icon={<FileText className="w-5 h-5" />} color="violet" />
                <MetricCard title="Chờ phê duyệt" value={stats.pending_approval} icon={<Clock className="w-5 h-5" />} color="amber"
                    trend={stats.pending_approval > 0 ? "Cần xử lý" : "Không có"} trendColor={stats.pending_approval > 0 ? "text-amber-600" : "text-emerald-600"} />
                <MetricCard title="Nhà cung cấp" value={stats.active_suppliers} icon={<Users className="w-5 h-5" />} color="slate" />
                <MetricCard title="Giá trị tháng" value={formatCurrency(stats.monthly_value)} icon={<TrendingUp className="w-5 h-5" />} color="emerald" subtitle="Tháng hiện tại" />
            </div>

            {/* Tabs */}
            <div className="bg-white p-2 rounded-[24px] border border-slate-200 shadow-sm flex items-center gap-1">
                {[
                    { id: "orders", label: "Đơn mua hàng", icon: FileText },
                    ...
                    <>
                        <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4">
                            <div className="relative flex-1 max-w-md w-full group">
                                ...
                                <input type="text" placeholder="Tìm PO / Nhà cung cấp..."
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="bg-slate-50/50 border-b border-slate-100">
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Đơn hàng</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Nhà cung cấp</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-right">Giá trị</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Trạng thái</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Tiến độ nhận</th>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {isLoading ? (
                                        ...
                                        <tr><td colSpan={6} className="py-32 text-center">
                                            ...
                                            <p className="text-sm font-black text-slate-400 uppercase tracking-widest">Chưa có đơn mua hàng</p>
                                                    ...
                                                    <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        ...
                                                        {(po.status === "APPROVED" || po.status === "SENT" || po.status === "PARTIALLY_RECEIVED") && (
                                                            <button onClick={() => handleReceive(po)}
                                                                ...
                                                                <ArrowDownLeft className="w-3 h-3" /> Nhận hàng
                                                            </button>
                                                        )}
                                                    </div>
                                                </td>
                ...
                {activeTab === "suppliers" && (
                    <>
                        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                            <h3 className="text-lg font-black text-slate-900">Danh sách Nhà cung cấp</h3>
                            <button onClick={() => setIsSupplierOpen(true)}
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="bg-slate-50/50 border-b border-slate-100">
                                        ...
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Mã số thuế</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Trạng thái</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-50">
